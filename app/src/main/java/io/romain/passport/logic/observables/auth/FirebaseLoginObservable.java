...
public class FirebaseLoginObservable implements Observable.OnSubscribe<AuthResult> {
	...
	public static Observable<AuthResult> create(Activity context, String user, String password) {
		return Observable.create(new FirebaseLoginObservable(context, user, password));
	}

	private FirebaseLoginObservable(Activity context, String user, String password) {
	...
	public void call(Subscriber<? super AuthResult> subscriber) {
		...
		FirebaseAuth.getInstance()
				.signInWithEmailAndPassword(mUser, mPassword)
